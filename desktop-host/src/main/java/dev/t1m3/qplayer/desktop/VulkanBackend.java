...
final class VulkanBackend implements GraphicsBackend {
    ...
    public byte[] snapshotPng() {
        if (currentIndex < 0 || images.length == 0) return null;
        long image = images[currentIndex];
        // Force Skija's recorded draw to the GPU (snapshot runs before present()'s
        // flush), then copy the image to a host-visible buffer and encode CPU-side
        // (makeImageSnapshot doesn't support wrapped Vulkan render targets here).
        context.flush();
        context.submit(true);
        try (MemoryStack s = stackPush()) {
            long size = (long) width * height * 4;
            // Staging buffer (host-visible).
            VkBufferCreateInfo bci = VkBufferCreateInfo.calloc(s).sType$Default()
                    .size(size)
                    .usage(VK_BUFFER_USAGE_TRANSFER_DST_BIT)
                    .sharingMode(VK_SHARING_MODE_EXCLUSIVE);
            LongBuffer pBuf = s.mallocLong(1);
            check(vkCreateBuffer(device, bci, null, pBuf), "vkCreateBuffer");
            long buffer = pBuf.get(0);
            VkMemoryRequirements req = VkMemoryRequirements.malloc(s);
            vkGetBufferMemoryRequirements(device, buffer, req);
            VkMemoryAllocateInfo mai = VkMemoryAllocateInfo.calloc(s).sType$Default()
                    .allocationSize(req.size())
                    .memoryTypeIndex(findMemoryType(req.memoryTypeBits(),
                            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, s));
            LongBuffer pMem = s.mallocLong(1);
            check(vkAllocateMemory(device, mai, null, pMem), "vkAllocateMemory");
            long memory = pMem.get(0);
            vkBindBufferMemory(device, buffer, memory, 0);

            // image COLOR_ATTACHMENT_OPTIMAL -> TRANSFER_SRC, copy, -> back.
            ...
            PointerBuffer pData = s.mallocPointer(1);
            vkMapMemory(device, memory, 0, size, 0, pData);
            java.nio.ByteBuffer mapped = MemoryUtil.memByteBuffer(pData.get(0), (int) size);
            boolean bgra = vkFormat == VK_FORMAT_B8G8R8A8_UNORM;
            java.awt.image.BufferedImage out =
                    new java.awt.image.BufferedImage(width, height, java.awt.image.BufferedImage.TYPE_INT_ARGB);
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    int i = (y * width + x) * 4;
                    int c0 = mapped.get(i) & 0xFF, c1 = mapped.get(i + 1) & 0xFF;
                    int c2 = mapped.get(i + 2) & 0xFF, a = mapped.get(i + 3) & 0xFF;
                    int r = bgra ? c2 : c0, g = c1, b = bgra ? c0 : c2;
            ...
            Logger.warn("vulkan snapshot failed: {}", t);
            return null;
        }
    }

    private int findMemoryType(int typeBits, int props, MemoryStack s) {
    ...
    private void copyImageToBuffer(long image, long buffer) {
        try (MemoryStack s = stackPush()) {
            VkCommandBuffer cmd = beginOneShot(s);
            VkBufferImageCopy.Buffer region = VkBufferImageCopy.calloc(1, s)
