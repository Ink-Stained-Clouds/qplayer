...
final class TrayController implements PlayerController.PlaybackListener {
    ...
    private boolean installAwt() {
        ...
        try {
            // Scale the source image to physical tray-slot size using bicubic
            // resampling so the icon is sharp on HiDPI displays.
            // setImageAutoSize(true) uses nearest-neighbour and produces a blurry
            // result when downsampling from a large source like our 192×192 icon.
            Image img = scaledForTray(loadSourceImage());
            ...
            // No setImageAutoSize: we provide a pre-scaled image.
    ...
    /** Pin a CJK font family; macOS default (Helvetica Neue) lacks CJK glyphs. */
    ...
    private BufferedImage loadSourceImage() {
        if (iconPng != null) {
            try {
    ...
    /**
     * Scale {@code src} to the physical pixel size of the system-tray slot using
     * bicubic interpolation. Sharper than {@code setImageAutoSize(true)} which
     * uses nearest-neighbour and produces a blurry result on a large source image.
     */
    ...
    /** Encode the placeholder to PNG bytes for native backends (WinTray/LinuxTray). */
